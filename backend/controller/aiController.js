...
const resolveDocumentForUser = async (
  documentId,
  userId,
  { requireReady = true } = {},
) => {
  ...
  if (rawDoc.status === "error") {
    return {
      error: {
        statusCode: 400,
        error:
          "This document failed to process and cannot be used with AI features yet.",
      },
    };
  }
  ...
  if (rawDoc.status !== "ready") {
    return {
      error: {
        statusCode: 400,
        error:
          "Document is not ready yet. Please refresh or upload a new file.",
      },
    };
  }
...
export const generateFlashcards = async (req, res, next) => {
  try {
    ...
    const { error, document } = await resolveDocumentForUser(
      documentId,
      req.user._id,
    );
    if (error) {
    ...
    if (!document.extractedText || document.extractedText.trim().length < 50) {
      return res.status(400).json({
        success: false,
        error:
          "Document does not have enough text content to generate flashcards.",
        statusCode: 400,
      });
    }
  ...
  try {
    ...
    if (extractedText.trim().length < 50) {
      return res.status(400).json({
        success: false,
        error:
          "Document is too short to generate a quiz. Please upload a document with more content.",
        statusCode: 400,
      });
    }
    ...
    if (!Array.isArray(questions) || questions.length === 0) {
      return res.status(400).json({
        success: false,
        error:
          "Could not generate any valid quiz questions for this document. Try a longer document.",
        statusCode: 400,
      });
    }
    ...
    const quiz = await Quiz.create({
      ...
      documentId: document._id,
      title:
        title || `${document.title || document.fileName || "Document"} - Quiz`,
    ...
    const populated = await Quiz.findOne({ _id: quiz._id }).populate(
      "documentId",
      "title fileName",
    );
